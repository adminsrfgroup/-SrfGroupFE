...
import { allSessionSelector, getNumberOfCarts, listConnectedUsersWebsocket } from '../../../user/store/slice';
import {
...
import { convertDateTimeToServer, isOnLine } from '../../../../shared/utils/utils-functions';
...
export default function DetailsOfffer() {
    ...
    const isLoadedSellDetailsOffersSelector = useSelector(isLoadedSellDetailsOffers) ?? false;
    const entitiesSellDetailsOffersSelector = useSelector(entitiesSellDetailsOffers) ?? [];
    const isLoadedRentDetailsOffersSelector = useSelector(isLoadedRentDetailsOffers) ?? false;
    const entitiesRentDetailsOffersSelector = useSelector(entitiesRentDetailsOffers) ?? [];
    const isLoadedFindDetailsOffersSelector = useSelector(isLoadedFindDetailsOffers) ?? false;
    const entitiesFindDetailsOffersSelector = useSelector(entitiesFindDetailsOffers) ?? [];
    ...
    React.useEffect(() => {
        if (entityPublicOfferSelector && entityPublicOfferSelector.offer && !loadingPublicOfferSelector) {
            ...
            if (entityPublicOfferSelector.offer.typeOffer === TypeOfferEnum.Sell && !isLoadedSellDetailsOffersSelector) {
                dispatch(fetchSellDetailsOffers({}));
            } else if (entityPublicOfferSelector.offer.typeOffer === TypeOfferEnum.Rent && !isLoadedRentDetailsOffersSelector) {
                dispatch(fetchRentDetailsOffers({}));
            } else if (entityPublicOfferSelector.offer.typeOffer === TypeOfferEnum.Find && !isLoadedFindDetailsOffersSelector) {
    ...
    const renderDialogReportOffer = () => {
        return (
            <Dialog open={openReportOfferModal} TransitionComponent={TransitionModal} keepMounted onClose={handleCloseReportOfferModal} aria-describedby="alert-dialog-slide-description">
                <DialogTitle>{t<string>('details_offer.title_dialog_report_offer')}</DialogTitle>
                <DialogContent>
                    <DialogContentText id="alert-dialog-slide-description">{t<string>('details_offer.description_dialog_report_offer')}</DialogContentText>
                </DialogContent>
                <DialogActions>
    ...
    const createConversation = (conversation: IConversationContent) => {
        if (currentUser.id !== entityPublicOfferSelector?.offer?.user?.id && isAuthenticated) {
    ...
    const handleCallbackAddComment = (content: string) => {
        if (content) {
            ...
            // Add event GA
            addEventGA(AllModulesEventGA.EventOffer.AddCommentOffer.eventName, AllModulesEventGA.EventOffer.AddCommentOffer.eventCategory, AllModulesEventGA.EventOffer.AddCommentOffer.eventLabel);
        }
    };
    ...
    const addNewEventGA = () => {
        addEventGA(AllModulesEventGA.EventOffer.ShareOffer.eventName, AllModulesEventGA.EventOffer.ShareOffer.eventCategory, AllModulesEventGA.EventOffer.ShareOffer.eventLabel);
    };
        ...
        <Box>
            <Container maxWidth="xl" className="details-offer-client">
                        ...
                        <Grid item xs={12} sm={6}>
                            ...
                            <Card sx={{ mb: 3 }}>
                                <CardContent>
                                    {entityPublicOfferSelector?.offer?.amount ? (
                                        <Typography
                                            ...
                                            color="secondary"
                                        >
                                            {entityPublicOfferSelector?.offer?.amount?.toLocaleString('tn-TN')} TND
                                        </Typography>
                                    ) : null}

                                    <Button variant="text" startIcon={<InfoOutlinedIcon />} color="secondary">
                                    ...
                                    {entityPublicOfferSelector?.offer?.startDate && entityPublicOfferSelector?.offer?.endDate ? (
                                        <Box>
                                            <Typography color="text.secondary" gutterBottom>
                                                ...
                                                :&nbsp;
                                                <strong>
                                                    <ConvertReactTimeAgo convertDate={entityPublicOfferSelector?.offer?.startDate} />
                                                </strong>
                                            </Typography>
                                            <Typography color="text.secondary" gutterBottom display="flex">
                                                ...
                                                :&nbsp;
                                                <strong>
                                                    <ConvertReactTimeAgo convertDate={entityPublicOfferSelector?.offer?.endDate} />
                                                </strong>
                                            </Typography>
                                        </Box>
                                    ) : null}
                                    ...
                                    {entityPublicOfferSelector?.offer?.typePeriodRent ? (
                                        <Typography color="text.secondary" gutterBottom display="flex">
                                            ...
                                            :&nbsp;
                                            <strong>
                                                {t<string>('common.type_periode_rent_' + entityPublicOfferSelector?.offer?.typePeriodRent)}
                                                &nbsp;
                                            </strong>
                                        </Typography>
                                    ) : null}
                                    ...
                                    <Box sx={{ mt: 2 }}>
                                        <Chip icon={<AccessTimeIcon />} label={<ConvertReactTimeAgo convertDate={entityPublicOfferSelector?.offer?.dateCreated} />} variant="outlined" sx={{ m: 1 }} />
                                        ...
                                        {entityPublicOfferSelector?.offer?.category ? (
                                            <Chip icon={<EmojiObjectsIcon />} label={t<string>('details_offer.label_category') + ' ' + getNameCategory()} variant="outlined" sx={{ m: 1 }} />
                                        ) : null}
                                    </Box>
                                ...
                                <CardActions disableSpacing>
                                    ...
                                    <IconButton sx={{ marginLeft: 'auto' }} onClick={reportOffer} color="error">
                            ...
                            {entityPublicOfferSelector?.offer?.typeContactClient !== OfferTypeContact.direct && entityPublicOfferSelector.offer?.typeOffer === TypeOfferEnum.Sell ? (
                                <CartSellDetailsOffer parentCallbackAddCart={addNewCart} loadingAddCart={loadingCartSelector} offer={entityPublicOfferSelector?.offer} />
                            ...
                            {entityPublicOfferSelector?.offer?.typeContactClient === OfferTypeContact.direct && entityPublicOfferSelector.offer?.typeOffer === TypeOfferEnum.Sell ? (
                                <Box sx={{ my: 3 }}>
                                    <Alert severity="warning">{t<string>('details_offer.offer_contact_direct')}</Alert>
                                </Box>
                            ) : null}
                        </Grid>
                ...
                <Box className="list-offer-relative" sx={{ mt: 5 }}>
                    {!loadingPublicOfferSelector ? (
                        <Typography variant="h4" gutterBottom className="uppercase" sx={{ textAlign: 'center' }}>
                            {t<string>('details_offer.similair_offers')}
                        </Typography>
                    ) : null}

                    {entityPublicOfferSelector?.offer?.typeOffer === TypeOfferEnum.Sell ? (
                        <ListRelatedDetailsOffer listOffers={entitiesSellDetailsOffersSelector} />
                    ) : entityPublicOfferSelector?.offer?.typeOffer === TypeOfferEnum.Rent ? (
                        <ListRelatedDetailsOffer listOffers={entitiesRentDetailsOffersSelector} />
                    ) : entityPublicOfferSelector?.offer?.typeOffer === TypeOfferEnum.Find ? (
                        <ListRelatedDetailsOffer listOffers={entitiesFindDetailsOffersSelector} />
                    ) : null}
                </Box>
            </Container>
