...
import { entitiesUserOffers, entityCountOffersByUser, fetchCountAllOffersByUser, fetchOffersByUser, loadingEntitiesUserOffers, loadingEntityCountOffersByUser } from '../../../offer/store/slice';
import { allSessionSelector, entityProfile, fetchProfileUser, loadingProfile, reportUser } from '../../store/slice';
import { ListOffersProfile } from './ui-segments/ListOffersProfile';
import { addConversation, loadingConversation } from '../../../chat/store/slice';
import { addFavoriteUsers, addSuccessFavoriteUser, resetFavoriteUsers } from '../../../favorite/store/slice';
...
export default function Profile() {
    ...
    const loadingEntityCountOffersByUserSelector = useSelector(loadingEntityCountOffersByUser) ?? false;
    const entityCountOffersByUserSelector = useSelector(entityCountOffersByUser) ?? false;

    const formik = useFormik({
        initialValues,
        validationSchema: validationSchemaAddMessage,
        onSubmit: (values) => {
            if (currentUser.id !== entityProfileSelector?.user?.id && isAuthenticated) {
    ...
    const renderDialogAddMessage = () => {
        return (
            <React.Fragment>
                <Dialog open={openAddMessageModal} TransitionComponent={TransitionModal} keepMounted onClose={handleClickCancelAddMessageModal} aria-describedby="alert-dialog-slide-description">
                    <form onSubmit={formik.handleSubmit}>
                        <DialogTitle>{t<string>('profile.title_send_message')}</DialogTitle>
                        <DialogContent>
                            <DialogContentText id="alert-dialog-slide-description">{getFullnameUser(currentUser)}</DialogContentText>
                            <FormControl
                                fullWidth
                                sx={{
                                    mt: 3,
                            ...
                            >
                                <InputLabel htmlFor="outlined-adornment-amount">{t<string>('profile.write_your_message')}</InputLabel>
                                <OutlinedInput
                                    id="content"
                                    name="content"
                                    label={t<string>('entityProfileSelector?.user?.write_your_message')}
    ...
    const handleAddFavoriteModal = () => {
        if (!isFavoriteUser && entityProfileSelector?.user?.id !== currentUser?.id) {
            if (isAuthenticated) {
    ...
    const renderDialogFavoriteUser = () => {
        return (
            <Dialog open={openFavoriteModal} TransitionComponent={TransitionModal} keepMounted onClose={() => setOpenFavoriteModal(false)} aria-describedby="alert-dialog-slide-description">
    ...
    const renderDialogReportUser = () => {
        return (
            <Dialog open={openReportUserModal} TransitionComponent={TransitionModal} keepMounted onClose={() => setOpenReportUserModal(false)} aria-describedby="alert-dialog-slide-description">
                <DialogTitle>{t<string>('profile.report_user')}</DialogTitle>
                <DialogContent>
                    <DialogContentText id="alert-dialog-slide-description">{t<string>('profile.description_report_user')}</DialogContentText>
                </DialogContent>
                <DialogActions>
    ...
    const addNewEventGA = () => {
        addEventGA(AllModulesEventGA.EventUser.Profile.eventName, AllModulesEventGA.EventUser.Profile.eventCategory, AllModulesEventGA.EventUser.Profile.eventLabel);
    };
                ...
                >
                    <Grid item xs={12} sm={6}>
                        <Breadcrumbs aria-label="breadcrumb">
                            ...
                            <Typography color="text.primary">{getFullnameUser(entityProfileSelector?.user)}</Typography>
                        </Breadcrumbs>
                    </Grid>
                </Grid>
                            ...
                            <Box>
                                <Paper elevation={3} sx={{ p: 1 }}>
                                    <Avatar
                                        ...
                                        src={getUserAvatar(entityProfileSelector?.user?.id, entityProfileSelector?.user?.imageUrl, entityProfileSelector?.user?.sourceConnectedDevice)}
                                        sx={{
                                            width: 80,
                                            height: 80,
                                            marginLeft: 'auto',
                                    ...
                                    <h4>{loadingProfileSelector ? <Skeleton animation="wave" height={30} /> : <React.Fragment>{getFullnameUser(entityProfileSelector?.user)}</React.Fragment>}</h4>

                                    {hasUserAuthority(entityProfileSelector?.user?.authorities, EAuthority.ROLE_MODERATOR) ? (
                                        <Button color="neutral" size="small" endIcon={<VerifiedIcon fontSize="small" color="error" />}>
                                            Super Admin
                                        </Button>
                                    ) : hasUserAuthority(entityProfileSelector?.user?.authorities, EAuthority.ROLE_ADMIN) ? (
                                    ...
                                    <Toolbar sx={{ justifyContent: 'center' }}>
                                        <Button
                                            ...
                                            disabled={currentUser.id === entityProfileSelector?.user?.id}
                                        >
                                    ...
                                    <Toolbar sx={{ justifyContent: 'center' }}>
                                        <Button
                                            ...
                                            disabled={isFavoriteUser || entityProfileSelector?.user?.id === currentUser?.id}
                                        >
                        ...
                        {!loadingProfileSelector ? (
                            <Box>
                                <Card sx={{ my: 3 }}>
                                    ...
                                    <CardActions disableSpacing>
                                        <CustomShare url="https://github.com/nygardk/react-share/blob/master/demo/Demo.tsx">
                                            <Tooltip title={<React.Fragment>{t<string>('common.label_share')}</React.Fragment>}>
                                                <ShareIcon onClick={addNewEventGA} />
                                            </Tooltip>
                                        </CustomShare>

                                        {!hasUserAuthority(entityProfileSelector?.user?.authorities, EAuthority.ROLE_MODERATOR) ? (
                                            <IconButton sx={{ marginLeft: 'auto' }} color="error" onClick={() => handleOpenReportUserModal()}>
                                                <Tooltip title={<React.Fragment>{t<string>('common.label_report')}</React.Fragment>}>
                                                    <FlagIcon />
                                                </Tooltip>
                                            </IconButton>
                                        ) : null}
                                ...
                                {!loadingEntityCountOffersByUserSelector ? (
                                    <Box>
                                        <StatisticOffers countOffersByUser={entityCountOffersByUserSelector} t={t} />
                                    </Box>
                                ) : null}
                        ...
                        <Slide direction="right" in={startAnimation}>
                            <div>
                                <Paper elevation={3} sx={{ p: 2, mb: 3 }}>
                                    <Box sx={{ mt: 2 }}>
                                        <h5>{t<string>('profile.personal_details')}</h5>
                                                        ...
                                                        <ListItemText
                                                            ...
                                                            secondary={
                                                                <React.Fragment>
                                                                    ...
                                                                    {loadingProfileSelector ? <Skeleton animation="wave" height={30} /> : entityProfileSelector?.user?.firstName}
                                                                </React.Fragment>
                                                            }
                                                        />
                                                    </ListItem>
                                                        ...
                                                        <ListItemText
                                                            ...
                                                            secondary={
                                                                <React.Fragment>
                                                                    ...
                                                                    {loadingProfileSelector ? <Skeleton animation="wave" height={30} /> : entityProfileSelector?.user?.lastName}
                                                                </React.Fragment>
                                                            }
                                                        />
                                                    </ListItem>
                                                ...
                                                <Grid item xs={12} md={6}>
                                                    <ListItem alignItems="flex-start">
                                                        <ListItemText
                                                            ...
                                                            secondary={
                                                                <React.Fragment>
                                                                    {loadingProfileSelector ? <Skeleton animation="wave" height={30} /> : entityProfileSelector?.user?.email}
                                                                </React.Fragment>
                                                            }
                                                        />
                                                    </ListItem>
                                                ...
                                                <Grid item xs={12} md={6}>
                                                    ...
                                                    >
                                                        <ListItemText
                                                            ...
                                                            secondary={
                                                                <React.Fragment>
                                                                    {loadingProfileSelector ? <Skeleton animation="wave" height={30} /> : entityProfileSelector?.user?.phone}
                                                                </React.Fragment>
                                                            }
                                                        />
                                                    </ListItem>
                                                            ...
                                                            secondary={
                                                                <React.Fragment>
                                                                    {loadingProfileSelector ? (
                                                                        ...
                                                                        entityProfileSelector?.user?.address?.city + ', ' + entityProfileSelector?.user?.address?.country
                                                                    ) : null}
                                                                </React.Fragment>
                                                            }
                                                        />
                                                ...
                                                <Grid item xs={12} md={6}>
                                                    <ListItem alignItems="flex-start">
                                                        <ListItemText
                                                            ...
                                                            secondary={
                                                                <React.Fragment>
                                                                    <a href={entityProfileSelector?.user?.linkProfileFacebook || ''} target={'_blank'} rel="noreferrer">
                                                                        Facebook
                                                                    </a>
                                                                </React.Fragment>
                                                            }
